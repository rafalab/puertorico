...
#'
#' @param dat data as downloaded from USCB
#' @param municipio TRUE or FALSE
#' @param year_input years in dat
#' @return  data.table

wrangle_pep <- function(dat, municipio = NULL, year_input) {
  
  DATE_DESC = gender = NAME = YEAR = CTYNAME = age = ageRange = agegroup = NULL # due to NSE notes in R CMD check
  ...
  if (year_input < 2020) {
    ...
    dat <- stats::setNames(dat, c( "municipio", "age", "M", "F"))
    dat <- melt(dat,
